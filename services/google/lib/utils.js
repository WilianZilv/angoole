...
var _process$env = process.env,
    ...
    GOOGLE_IT_RESULT_STATS_SELECTOR =
        _process$env.GOOGLE_IT_RESULT_STATS_SELECTOR,
...
var saveToFile = function saveToFile(output, results) {
    if (output !== undefined) {
        fs.writeFile(output, JSON.stringify(results, null, 2), "utf8", function(
            err
        ) {
            if (err) {
                console.error(
                    "Error writing to file ".concat(output, ": ").concat(err)
                );
            }
        });
    }
